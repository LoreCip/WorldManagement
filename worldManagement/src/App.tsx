...
const NavIconButton: React.FC<NavIconButtonProps> = ({ icon, title, isActive, onClick, pinToBottom }) => (
	<button
		onClick={onClick}
		title={title}
		style={{
	...
	return (
		<div style={{ display: "flex", height: "100vh", width: "100vw", overflow: "hidden", backgroundColor: colors.bgVoid }}>
			{activeTab !== "hub" && (
				...
				>
					<NavIconButton icon="🏠" title={t("app.nav.hub")} isActive={false} onClick={() => handleTabChange("hub")} />
			...
			<div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
				{activeTab === "hub" && (
					<HubView onNavigate={handleHubNavigate} onOpenSettings={() => handleTabChange("settings")} />
				)}
				...
				{activeTab === "maps" && <MapView onOpenArticle={handleOpenArticle} initialMapId={selectedMapId} />}
				...
				{activeTab === "relations" && (
					<RelationsView onNavigateToWiki={handleOpenArticle} onNavigateToCharacterSheet={handleOpenCharacterSheet} />
				)}
