	...
	(
		...
		const clusters = useMemo<Cluster[]>(() => {
			...
			for (const ev of sorted) {
				...
				if (last && x - valueToX(last.items[last.items.length - 1].time_value) < CLUSTER_THRESHOLD_PX) {
					last.items.push(ev);
				} else {
		...
		const selectedLocalAnchor = useMemo<{ x: number; y: number; preferredSide: "above" | "below" } | null>(() => {
			...
			if (laned) {
				...
				const rawX = visibleX2 > visibleX1 ? (visibleX1 + visibleX2) / 2 : (laned.x1 + laned.x2) / 2;
			...
			for (const cluster of clusters) {
				...
				if (idx < MAX_POINT_LANES) {
					return { x: cluster.x, y: axisY + POINT_LANE_BASE + idx * POINT_LANE_STEP, preferredSide: "below" };
				}
				return { x: cluster.x, y: axisY + POINT_LANE_BASE + MAX_POINT_LANES * POINT_LANE_STEP, preferredSide: "below" };
			}
		...
		const handleWheel = (e: React.WheelEvent) => {
			...
			const newPixelsPerDay = clamp(pixelsPerDay * zoomFactor, MIN_PIXELS_PER_DAY, MAX_PIXELS_PER_DAY);
			...
			>
				<svg width={width} height={height} style={{ display: "block" }}>
					...
					{ticks.map((tick) => {
						...
						return (
							<g key={tick.value}>
								<line x1={x} y1={axisY - 6} x2={x} y2={axisY + 6} stroke={colors.borderSubtle} strokeWidth={1} />
								<text x={x} y={axisY + 24} fill={colors.textFaint} fontSize={11} fontFamily={fonts.body} textAnchor="middle">
									{tick.label}
								</text>
							</g>
						);
					...
					{lanedBars.map(({ event: ev, x1, x2, lane }) => {
						...
						const label = `${ev.category_icon ? ev.category_icon + " " : ""}${ev.title.length > 26 ? ev.title.slice(0, 24) + "…" : ev.title
							}`;
						...
						return (
							...
							>
								<line x1={x1} y1={barBottom} x2={x1} y2={axisY} stroke={`${colors.gold}55`} strokeWidth={1.2} />
								<rect
									x={Math.min(x1, x2)}
									y={barTop}
					...
					{clusters.map((cluster, clusterIdx) => {
						...
						if (cluster.items.length === 1) {
							...
							const color = ev.category_color ?? colors.gold;
							const label = `${ev.category_icon ? ev.category_icon + " " : ""}${ev.title.length > 22 ? ev.title.slice(0, 20) + "…" : ev.title
								}`;
							...
							return (
								...
								>
									<line
										x1={cluster.x} y1={axisY} x2={cluster.x} y2={markerY}
										stroke={isSelected ? colors.gold : `${colors.gold}55`}
										strokeWidth={isSelected ? 2 : 1.2}
									/>
									<circle
										cx={cluster.x} cy={markerY} r={isSelected || isHovered ? 7 : 5.5}
										fill={isSelected ? color : colors.bgPanelRaised}
										stroke={color}
										strokeWidth={isSelected ? 0 : 1.8}
									/>
									<TextChip
										x={cluster.x + 14} y={markerY + 4} text={label}
										color={isSelected ? colors.gold : colors.textSecondary}
										fontWeight={isSelected ? 700 : 500} anchor="start"
									/>
									{isHovered && !isSelected && (
										<TextChip
											x={cluster.x + 14} y={markerY + 20}
											text={formatTimeValue(ev.time_value, ev.precision)}
											color={colors.textFaint} fontWeight={400} fontSize={10} anchor="start"
										/>
									)}
								</g>
							);
						...
						if (!isDayResolution) {
							...
							return (
								<g
									...
									style={{ cursor: "pointer" }}
								>
									<line x1={cluster.x} y1={axisY} x2={cluster.x} y2={markerY} stroke={`${colors.gold}55`} strokeWidth={1.2} />
									<circle cx={cluster.x} cy={markerY} r={11} fill={colors.bgPanelRaised} stroke={colors.gold} strokeWidth={2} />
									<text x={cluster.x} y={markerY + 4} fill={colors.gold} fontSize={11} fontWeight={700} fontFamily={fonts.body} textAnchor="middle">
							...
							<g key={`cluster-${cluster.minValue}-${cluster.maxValue}-${clusterIdx}`}>
								...
								{overflowCount > 0 && (() => {
									...
									return (
										...
										>
											<line x1={cluster.x} y1={axisY} x2={cluster.x} y2={markerY} stroke={`${colors.gold}55`} strokeWidth={1.2} />
											<circle cx={cluster.x} cy={markerY} r={10} fill={colors.bgPanelRaised} stroke={colors.gold} strokeWidth={2} />
											<text x={cluster.x} y={markerY + 4} fill={colors.gold} fontSize={11} fontWeight={700} fontFamily={fonts.body} textAnchor="middle">
												+{overflowCount}
											</text>
										</g>
				...
				{openOverflow && (
					...
					>
						{openOverflow.items.map((ev) => (
							...
							>
								{ev.category_icon ? `${ev.category_icon} ` : ""}{ev.title}
								<span style={{ color: colors.textFaint, marginLeft: "0.4rem", fontSize: "0.72rem" }}>
