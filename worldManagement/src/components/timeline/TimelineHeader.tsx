      ...
      <Toolbar>
        ...
        <Popover
          ...
          trigger={
            ...
            >
              {t("timeline.header.filters")}
              {activeCategoryIds ? ` (${activeCategoryIds.size})` : ""}
            </ToolbarButton>
          }
        >
          {categories.map((cat) => {
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: "0.4rem" }}>
            <ToolbarButton onClick={onClearCategoryFilters}>
        ...
        >
          <div style={{ display: "flex", gap: "0.4rem" }}>
            <input
              type="number"
              value={todayInput.year}
              placeholder={t("timeline.balloon.year")}
              onChange={(e) =>
                setTodayInput({ ...todayInput, year: parseInt(e.target.value || "0", 10) })
              }
              style={{ ...smallInput, width: "70px" }}
            />
            <input
              type="number"
              value={todayInput.month}
              min={1}
              max={12}
              placeholder={t("timeline.balloon.month")}
              onChange={(e) =>
                setTodayInput({ ...todayInput, month: parseInt(e.target.value || "1", 10) })
              }
            ...
            <input
              ...
              onChange={(e) =>
                setTodayInput({ ...todayInput, day: parseInt(e.target.value || "1", 10) })
              }
              style={{ ...smallInput, width: "55px" }}
            />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: "0.5rem" }}>
            ...
            <ToolbarButton
              onClick={() => {
                onSetToday(timeInputToValue(todayInput));
                setShowTodayPicker(false);
              }}
            >
              {t("common.save")}
