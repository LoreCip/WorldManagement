...
export function useWiki() {
	...
	useEffect(() => {
		...
		(async () => {
			const [sheetId, mapId] = await Promise.all([
				invokeSafe<string | null>("get_character_sheet_id_by_article", { articleId: currentArticle.id }),
	...
	const handleNavigateToTitle = useCallback(
		async (title: string) => {
			...
			console.log("navigateToTitle: match trovato?", target ? target.id : "NESSUNO", "articoli in memoria:", articles.length);
			if (target) {
			...
			const createNew = window.confirm(`La voce "${title}" non esiste ancora. Vuoi crearla adesso?`);
			if (!createNew) return;
